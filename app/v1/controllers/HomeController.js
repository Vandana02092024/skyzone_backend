...
const getLatLong = async (zipcode) => {
    ...
    try {
        ...
        console.log('geocode res returned', res);

        // if (Array.isArray(res) && res.length > 0) {
        //     return { latitude: res[0].latitude, longitude: res[0].longitude };
        // }
        // Filter results to only return the one with the US country code or country name
        const usResult = res.find(result => result.countryCode === 'US' || result.country === 'United States');

        if (usResult) {
            return { latitude: usResult.latitude, longitude: usResult.longitude };
        }
...
const getNearbyLocation = async (latitude, longitude,limit) => {
    ...
    acos(
      ...
      sin(radians(${latitude})) * sin(radians(latitude))
        ...
        // having: Sequelize.literal('distance <= 1'),
...
export const searchNearbyLocations = async (req, res) => {
    ...
    try {
        if(city) {
            const { latitude, longitude } = await getLatLong(city);
            if(!latitude ||!longitude) {
                return res.status(400).json(CommonFunction.errMessage("Invalid zipcode"));
            }
            results = await getNearbyLocation(latitude, longitude, limit);
        }
        else if (!(latitude && longitude) || latitude === "undefined" || longitude === "undefined") {
            results = await searchLocations({ state }, limit, false);
...
export const getClientDetails = async (req, res) => {
    try {
        ...
        const clientDetails = await ClientMaster.findOne({
            ...
            [Sequelize.literal('website'), 'membership_cancellation'],
        ],
            where: { client_id },
        });
