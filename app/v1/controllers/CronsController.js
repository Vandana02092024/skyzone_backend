...
const PushNotification = async (req, res) => {
    try {
        ...
        console.log('notificationDt', notificationDt);
        // return false;
            ...
            notificationDt.map(async (data) => {
                try {
                    ...
                    if (currentTime >= notificationTime) {
                        ...
                        if(notify){
                            console.log('here2');
                            let clientIds = data.client_ids.split(',').map(Number);
                            let getTopics = await ClientMaster.findAll({
                                attributes: ['client_id', 'topic_name'],
                                where: {
                                  client_id: {
                                    [Op.in]: clientIds
                                  }
                            ...
                            let allTopics = getTopics
                            .map(topic => `'${topic.topic_name}' in topics`)
                            .join(' || ');
                            // let topic = 'userRegistration';
                            let notitymessage = decode(data.message);
                            await firebaseService.notifyMessagetoMultiDevices(data.title, notitymessage, allTopics);
