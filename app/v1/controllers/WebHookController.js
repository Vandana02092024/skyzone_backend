...
import { Webhooks, Customers, BookingItems, Ticket, MembershipRedemptions, MobileCustomerMemberships } from "../../../config/tables.js";
...
const Booking = async (req, res) => {
    try {
        ...
        if(response.eventType===3 && response.data.booking.status==='Cancelled') {
            const bookingReference = response.data.booking.bookingReference;
            if (bookingReference) {
                try {
                    await MobileCustomerMemberships.update(
                        {
                            status: 0, 
                            cancel_date: new Date(), // Current date and time
                            is_webhook: 'true'      // Flag indicating update via webhook
                        },
                        { where: { bookingReference } }
                    );
                    console.log('Membership status updated to cancelled.');
                } catch (error) {
