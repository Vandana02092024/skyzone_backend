...
const addGeneralQuery = async (req, res) => {
    ...
    if (!data.username) return res.status(400).json(CommonFunction.errMessage("Username is required."));
    ...
    if (!emailRegex.test(username) && !phoneRegex.test(username)) {
        return res.status(400).json(CommonFunction.errMessage("Please enter a valid username."));
    }
