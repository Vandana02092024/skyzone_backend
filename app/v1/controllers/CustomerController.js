...
export const CustomerPreferedLocation = async (req, res) => {
    try {
        const customer_id = CommonFunction.getCustomerId(req, res);
        const data = req.body;
        if (!customer_id) {
        ...
        const manageLocation = await firebaseService.RegisterDeviceIdRegRem(data,customer_id);
        
        if (manageLocation.code) {
            return res.status(200).json(CommonFunction.succsMessage("success", []));
        } else {
            return res.status(200).json(CommonFunction.errMessage("There is some issue"));
        }
    } catch (error) {
        console.error("Error in CustomerPreferedLocation:", error);
        return res.status(500).json(CommonFunction.errMessage("Internal Server Error"));
    }
};
