...
const PaymentForBooking = async (req, res) => {
  ...
  const items = await Promise.all(
    data.items.map(async (item) => {
      const checkGroupsize = await paymentService.getProduct(item.productId,data.client_id);
      const groupsize = checkGroupsize.groupSize;
      return {
  ...
  const rollerResponse = await rollerUtils.postRequest(endpoint, roller.accessToken, { items });

  console.log('rollerResponse for capicity reservation',rollerResponse);

  if (rollerResponse.errors) {
    const rollerError = rollerResponse.errors[0]?.message;
    // return res.status(500).send(`Capacity Reservation Error: ${JSON.stringify(rollerResponse.errors)}`);
    return res.status(400).json(CommonFunction.errMessage(rollerError));
...
const MembershipBooking = async (req, res) => {
  ...
  if(continueBooking) {
    if(fl) {
      return res.status(400).json(CommonFunction.errMessage('Multiple pass bookings detected within a minute.'));
    }
    else {
      thrott = true;
    }
 ...
 // CREATE SUBSCRIPTION FOR THE MEMBERSHIP PLAN
  ...
  if(thrott) {
    bkId = continueBooking?.booking_id;
  } else {
    bkId = await paymentService.generateBookingId(data.client_id);
  }
  ...
  if (fl) {
    ...
    let booking = (thrott) ? true : await paymentService.createBooking(data, customerData);
    if (booking) {
      return res.status(200).send({
        status: 'success',
        message: 'success',
