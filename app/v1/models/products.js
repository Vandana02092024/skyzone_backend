...
productService.getAvailableProducts = async (client_id,display=1) => {
  try {
    const whereCondition = {
      client_id: client_id,
      ...(display === 1 && { status: 1 }) // If `display` is 1, add `status` condition
    };
...
productService.getOfferDiscount = async (offer_id,client_id) => {
  try {
      const result = await LatestOfferings.findOne({
        attributes: ['id','title','camp_type','discount_code','discount_type','discount_amount','discount_percent','offer_type','reward_points','client_id'],
