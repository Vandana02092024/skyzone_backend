...
const MobileCustomers = sequelize.define(
  "mobile_customers",
  {
    ...
    refresh_token_expire: { type: DataTypes.DATE,allowNull: true },
...
const MobileCustomersPreferedLocation = sequelize.define(
  ...
  {
    tableName: "mobile_customers_prefered_location",
    timestamps: false,
  }
);
...
const FirebaseTokenRegLogs = sequelize.define('FirebaseTokenRegLogs', {
  id: {
    type: DataTypes.INTEGER,
    primaryKey: true,
    field: "id",
  ...
  tableName: 'firebase_token_reg_logs',
  timestamps: false // Since we are handling timestamps manually
});

const FirebaseTopicRegIds = sequelize.define('FirebaseTopicRegIds', {
  id: {
    type: DataTypes.INTEGER,
    primaryKey: true,
    field: "id",
  ...
  tableName: 'firebase_topic_reg_ids',
  timestamps: false // Since we are handling timestamps manually
});
...
ClientMaster.hasMany(UserLocations, { foreignKey: "client_id", as: "user_locations" });
UserLocations.belongsTo(ClientMaster, { foreignKey: "client_id" });
ClientMaster.hasMany(RestaurantOrder, { foreignKey: "client_id" });
...
BookingItems.belongsTo(RestProductPackageItem, {
  foreignKey: 'productId',
  targetKey: 'productId',
  as: 'packageItem'
});
...
MobileCustomerQueries.belongsTo(MobileCustomers, { foreignKey: 'customer_id' });
MobileCustomers.hasMany(MobileCustomerQueries, { foreignKey: 'customer_id' });
